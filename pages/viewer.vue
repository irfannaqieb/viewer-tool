...
const loadConfiguration = async (config) => {
	try {
		...
		// Convert config images to our format
		imageList.value = config.images.map((configImage, index) => {
			...
			if (configImage.gps && Array.isArray(configImage.gps)) {
				gpsCoordinates = {
					longitude: configImage.gps[0],
					latitude: configImage.gps[1],
					altitude: configImage.gps[2], // May be undefined
				};
			}
			...
			return {
				...
				northCalibration: northCalibration,
